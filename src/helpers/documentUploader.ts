...
export const uploadMultipleDocument = async (
    ...
    if (!fileArray?.length) {
        logger.warn("uploadMultipleDocument called with no files");
        return [];
    }
        ...
        const results = await Promise.all(
            fileArray.map(async (file) => {
                ...
                try {
                    ...
                    logger.debug(`Uploaded Document: ${res.secure_url}`);
...
export const uploadSingleDocument = async (file: Express.Multer.File): Promise<{url: string, public_id: string}> => {
  if (!file?.path) {
    throw createError(400, "File path missing; are you using memoryStorage?");
  }
  try {
    ...
    logger.info(`Successfully uploaded single document: ${result.secure_url}`);
    return {url: result.secure_url, public_id: result.public_id};
  } catch (err) {
    logger.error("Failed to upload single document to Cloudinary:", err);
    throw createError(400, "Failed to upload single document; please try again");
  } finally {
    try {
      await fs.unlink(file.path);
    } catch {
